...
mod single_label_multiline_message {
    use super::*;
        ...
        static ref TEST_DATA: TestData<'static, SimpleFiles<&'static str, String>> = {
            ...
            let diagnostics = vec![
                Diagnostic::error()
                    ...
                    .with_labels(vec![
                        Label::primary(file_id1, 71..72)
                            .with_message("second mutable borrow occurs here\nwhile `v` is still borrowed mutably"),
                        Label::secondary(file_id1, 64..65)
                            .with_message("first borrow later used by call"),
                        Label::secondary(file_id1, 66..70)
                            .with_message("first mutable borrow occurs here\nand stays borrowed mutably for the rest of the function call"),
                    ]),
                Diagnostic::error()
                    .with_message("aborting due to previous error")
                    .with_notes(vec![
...
mod multiline_overlapping_multiline_messages {
    use super::*;
        ...
        static ref TEST_DATA: TestData<'static, SimpleFile<&'static str, String>> = {
            ...
            let diagnostics = vec![
                Diagnostic::error()
                    ...
                    .with_labels(vec![
                        // this secondary label is before the primary label to test the locus calculation (see issue #259)
                        Label::secondary((), 89..134).with_message("this is found to be of type:\n`Result<ByteIndex, LineIndexOutOfBoundsError>`"),
                        Label::secondary((), 81..85).with_message("this is less\nless is less than more"),
                        Label::secondary((), 71..79).with_message("this is ordering\nit denotes an ordering"),
                        Label::primary((), 230..351).with_message("expected enum `Result`, found struct:\n`LineIndexOutOfBoundsError`"),
                        Label::secondary((), 167..195).with_message("this is found to be of type `Result<ByteIndex, LineIndexOutOfBoundsError>`"),
                        Label::secondary((), 8..362).with_message(unindent::unindent(
                        "
...
mod suggestion_replace_multiple {
    use super::*;
    ...
    lazy_static::lazy_static! {
        static ref TEST_DATA: TestData<'static, SimpleFiles<&'static str, String>> = {
            ...
            let diagnostics = vec![
                Diagnostic::error()
                    .with_message("this is a block expression, not an array")
                    .with_labels(vec![
                        Label::primary(file_id, 41..52)
                    ])
                    .with_subdiagnostics(vec![
                       Subdiagnostic::Suggestion(Suggestion {
                            file_id,
        ...
        static ref TEST_DATA: TestData<'static, SimpleFiles<&'static str, String>> = {
            ...
            let diagnostics = vec![
                Diagnostic::warning()
                    .with_message("thingy")
                    .with_labels(vec![
                        Label::primary(file_id, 0..5).with_message("1")
                    ])
                    .with_subdiagnostics(vec![
                        Subdiagnostic::SpannedNote(SpannedNote {
                            severity: Severity::Help,
                            message: format!("due to this"),
                            labels: vec![
                                Label::primary(file_id, 7..14).with_message("2")
                            ],
                        }),
                    ])
            ];
...
mod suggestion_multiple_lines {
    ...
    lazy_static::lazy_static! {
        static ref TEST_DATA: TestData<'static, SimpleFiles<&'static str, String>> = {
            ...
            let diagnostics = vec![
                Diagnostic::error()
                    .with_message("expected four items, found three")
                    .with_labels(vec![
                        Label::primary(file_id, 41..81)
                    ])
                    .with_subdiagnostics(vec![

    ...
    FileId: Copy + PartialEq,
{
    ...
    fn label_files<'files>(
        ...
        labels: &'diagnostic [Label<FileId>],
    ...
    ///   │
    /// ```
    fn render_source_snippets<'files>(
        &self,
        outer_padding: usize,
        files: &'files impl Files<'files, FileId = FileId>,
        ...
        while let Some(labeled_file) = labeled_files.next() {
            ...
            if labeled_files.peek().is_none()
                && self.diagnostic.notes.is_empty()
                && self.diagnostic.subdiagnostics.is_empty()
            {
    ...
    where
        FileId: 'files,
    {
        let (outer_padding, labeled_files) = self.label_files(files, &self.diagnostic.labels)?;
        // Header and message
        //
        // ```text
        ...
        // Subdiagnostics
        let mut multiline_suggestions = 0;
        let mut empty_suggestions = 0;
        for subdiagnostic in &self.diagnostic.subdiagnostics {
            match subdiagnostic {
                ...
                //
                // ```text
                // = note: blabla is defined here
                // | 2       let a: [bool; 2] = [true, false];
                // |             ^
                // ```
                Subdiagnostic::SpannedNote(SpannedNote {
                    ...
                    let (outer_padding, labeled_files) = self.label_files(files, labels)?;
                    renderer.render_header(None, *severity, None, message.as_str())?;
                    self.render_source_snippets(outer_padding, files, labeled_files, renderer)?;
                }
            }
        }

        if multiline_suggestions > 0 {
            renderer.render_snippet_note(
                outer_padding,
                &format!(
                    "(Note: ignored {} multi-line suggestion{})",
                    multiline_suggestions,
                    if multiline_suggestions == 1 { "" } else { "s" }
                ),
            )?;
        }
        ...
        if empty_suggestions > 0 {
            renderer.render_snippet_note(
                outer_padding,
                &format!(
                    "(Note: ignored {} empty suggestion{})",
                    empty_suggestions,
                    if empty_suggestions == 1 { "" } else { "s" },
                ),
            )?;
        }

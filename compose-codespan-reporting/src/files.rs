...
pub enum Error {
    ...
    /// The given slice is contained in the file, but is not a valid UTF-8 string due to its
    /// boundaries.
    InvalidSlice { given: (Bound<usize>, Bound<usize>) },
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Error::InvalidSlice { .. } => write!(f, "slice endpoint is not a code point boundary"),
...
///
/// ```rust
/// use compose_codespan_reporting::files;
///
/// let source = "\n\n🗻∈🌏\n\n";
///
